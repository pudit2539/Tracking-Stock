...
function populateSettingsForm() {
  ...
  if (document.getElementById('setting-target-id')) document.getElementById('setting-target-id').value = s.line_target_id || '';
...
async function saveSettings(e) {
  ...
  const payload = {
    ...
    line_target_id: document.getElementById('setting-target-id').value,
...
// Trigger Manual LINE Notifications
async function triggerSendTestMessage() {
  const targetId = document.getElementById('setting-target-id')?.value.trim();
  if (targetId && !/^[UCRucr][0-9a-fA-F]{32}$/.test(targetId) && !targetId.startsWith('U') && !targetId.startsWith('C') && !targetId.startsWith('R')) {
    showToast('⚠️ LINE Target ID ต้องเป็น User ID (ขึ้นต้นด้วย U...) หรือ Group ID (ขึ้นต้นด้วย C...) ไม่ใช่ LINE ID ทั่วไปครับ (ดูที่แท็บ Basic settings)', 'error');
    return;
  }

  showToast('กำลังส่งข้อความทดสอบไปยัง LINE...');
  try {
    const res = await fetch('/api/line/test', {
      ...
      body: JSON.stringify({
        ...
        target_id: document.getElementById('setting-target-id')?.value || undefined
      })
    });
    const json = await res.json();
    if (!json.success) throw new Error(json.error);
    showToast('ส่งข้อความทดสอบเข้า LINE เรียบร้อยแล้ว!');
  } catch (err) {
...
async function triggerSendLiveReport() {
  const targetId = document.getElementById('setting-target-id')?.value.trim();
  if (targetId && !targetId.startsWith('U') && !targetId.startsWith('C') && !targetId.startsWith('R')) {
    showToast('⚠️ LINE Target ID ต้องขึ้นต้นด้วย U (ส่วนตัว) หรือ C/R (กลุ่ม) ดูได้ในแท็บ Basic settings', 'error');
    return;
  }

  showToast('กำลังส่งรายงานสต็อกไปยัง LINE...');
  try {
    ...
    const json = await res.json();
    if (!json.success) throw new Error(json.error);
    showToast('ส่งรายงานสต็อกเข้า LINE เรียบร้อยแล้ว!');
  } catch (err) {
    showToast('ส่งไม่สำเร็จ: ' + err.message, 'error');
  }
}
